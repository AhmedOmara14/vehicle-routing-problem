...
alfa = 2
beta = 5
iterations = 1000   #to reduce cost to find best cost
ro = 0.8 
vehicle = 4
sigm = 3 
th = 80
...
def generateGraph():
    ...
    for i in depotcustomer:
        vertices.remove(i)
        #to grouping and make multidepot    
        #distances  ((x0-y0)+(x1-y1))^2   distance betwwen two point to find shortest path
...
def solutionOfOneAnt(vertices, edges, capacityLimit, demand, pheromones):
    ...
    while(len(vertices)!=0):
        ...
        #to make tsp no visit same city again in each depot
        while(len(vertices)!=0):
...
def main():
    ...
    for i in range(iterations):
        ...
        for _ in range(vehicle):
            solution = solutionOfOneAnt(vertices.copy(), edges, capacityLimit, demand, pheromones)#update ant and move it
            solutions.append((solution, bestcost(solution, edges)))
